...
# parent directory's "src" directory.  Since it lives in tools/ in
# the Marabou source tree, that means src/ in the Marabou source tree.
#
...
my @searchdirs = ();
if($#ARGV == -1) {
  (chdir($dir."/..") && -f "src/engine/Marabou.h") || die "can't find top-level source directory for Marabou";
...
EOF
  ...
  $searchdirs[0] = 'src';
  $searchdirs[1] = 'maraboupy';
  $searchdirs[2] = 'regress';
} else {
  @searchdirs = @ARGV;
}
...
sub handleFile {
  ...
  open(my $AUTHOR, "$dir/tools/get-authors.sh " . $infile . '|');
