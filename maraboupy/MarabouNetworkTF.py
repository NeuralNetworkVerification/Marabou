...
class MarabouNetworkTF(MarabouNetwork.MarabouNetwork):
    ...
    def addOpEquations(self, op):
        """
        A function to generate equations corresponding to the addition of two tensors
        Arguments:
            op: (tf.op) representing an add operation
        """
        input_ops = [i.op for i in op.inputs]
        prevValues = [self.getValues(i) for i in input_ops]
        curValues = self.getValues(op).reshape(-1)
        for i in range(len(curValues)):
            e = MarabouUtils.Equation()
            for j in range(len(prevValues)):
                prevVal = prevValues[j].reshape(-1)
                e.addAddend(1., prevVal[i])
            e.addAddend(-1.,curValues[i]) # A[i] + B[i] = C[i] -becomes-> A[i] + B[i] - C[i] == 0
            e.setScalar(0.)
            self.addEquation(e)
