...
def test_evaluate_network():
    ...
    for i in range(N):
        ...
        output1 = nnet_object.evaluateNetworkToLayer(inputs, last_layer=-1, normalize_inputs=False,
                                                     normalize_outputs=False, activate_output_layer=False)

        output2 = nnet_object.evaluateNetwork(inputs,normalize_inputs=False,normalize_outputs=False)
        ...
        output1 = nnet_object.evaluateNetworkToLayer(inputs, last_layer=-1, normalize_inputs=True,
        ...
        output1 = nnet_object.evaluateNetworkFromLayer(layer_output, first_layer=layer)
...
def test_normalize_read_flag():
    ...
    for i in range(N):
        ...
        with_marabou_output = nnet_object_a.evaluate(np.array([inputs]), useMarabou=True).flatten()
        with_marabou_output_rounded = np.array([float(round(y, 8)) for y in with_marabou_output])
        output2_rounded = np.array([float(round(y, 8)) for y in output2])
        assert (output2_rounded == with_marabou_output_rounded).all()
