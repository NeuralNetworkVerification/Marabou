...
subprocess.run('python3 {0}/launcher.py --pyFile {0}/CnnAbsTB.py --net {0}/evaluation/networkA.h5 --prop_dist 0.03 --propagate_from_file --runs_per_type {1} --batchDir {2}'.format(CnnAbs.maraboupyPath, args.instances, graphDir).split(' '))
...
os.chdir(logDir)
subprocess.run('python3 {}/resultsParserSlurm.py --graph_dir_name {}'.format(CnnAbs.maraboupyPath, graphDir).split(' '))
os.chdir(cwd)
