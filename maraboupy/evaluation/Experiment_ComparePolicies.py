...
subprocess.run('python3 {0}/evaluation/launcher.py --abstract_first --pyFile {0}/CnnAbsTB.py --net {0}/evaluation/networkA.h5 --prop_dist 0.03 --propagate_from_file --runs_per_type {1} --batchDir {2}'.format(CnnAbs.basePath, args.instances, graphDir).split(' ') + (['--gtimeout', str(args.gtimeout)] if args.gtimeout != -1 else []), stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
