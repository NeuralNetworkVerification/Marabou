...
{
    if ( res == ( Minisat::lbool( (uint8_t)0 ) ) )
        return SAT_VALUE_TRUE;
    if ( res == ( Minisat::lbool( (uint8_t)2 ) ) )
...
void MinisatSatSolver::toSatClause( const Minisat::Clause &clause, SatClause &sat_clause )
{
                                   ...
                                   CVC4::context::UserContext *userContext )
{
    ...
    */
    ...
    // Create the solver
    d_minisat = new Minisat::SimpSolver( theoryProxy, d_context, userContext, true );
...
{
    // Trace("limit") << "SatSolver::solve(): have limit of " << resource << " conflicts" <<
    // std::endl;
    setupOptions();
    if ( resource == 0 )
    {
...
void MinisatSatSolver::getUnsatAssumptions( std::vector<SatLiteral> &unsat_assumptions )
{
...
void MinisatSatSolver::requirePhase( SatLiteral lit )
{
    ...
    // std::cout << "minisat: " << "requirePhase(" << lit << ")" << " " <<  lit.getSatVariable() <<
    // " " << lit.isNegated() << std::endl;
...
std::vector<SatLiteral> MinisatSatSolver::getDecisions() const
{
    ...
    const Minisat::vec<Minisat::Lit> &miniDecisions = d_minisat->getMiniSatDecisions();
...
void MinisatSatSolver::resetTrail()
{
    d_minisat->resetTrail();
}

} // namespace prop

template <> prop::SatLiteral toSatLiteral<Minisat::Solver>( Minisat::Solver::TLit lit )
{
