/*********************                                                        */
/*! \file Test_SparseUnsortedVectors.h
** \verbatim
...
class MockForSparseUnsortedVectors
{
public:
};

class SparseUnsortedVectorsTestSuite : public CxxTest::TestSuite
{
public:
    MockForSparseUnsortedVectors *mock;

    void setUp()
    {
        TS_ASSERT( mock = new MockForSparseUnsortedVectors );
    }
    ...
    void test_deletions()
    {
        ...
        sv1.commitChange( 1, 0, 0.0 );
        sv1.commitChange( 3, 1, 0.0 );

        // No changes before "execute" is called
        for ( unsigned i = 0; i < 4; ++i )
            for ( unsigned j = 0; j < 4; ++j )
                TS_ASSERT_EQUALS( sv1.get( i, j ), M1[i*4 + j] );

        TS_ASSERT_THROWS_NOTHING( sv1.executeChanges() );
        ...
        // Fake elements
        sv1.commitChange( 1, 3, 0.0 );
        sv1.commitChange( 3, 2, 0.0 );
        TS_ASSERT_THROWS_NOTHING( sv1.executeChanges() );
        ...
        sv1.commitChange( 1, 1, 0.0 );
        sv1.commitChange( 2, 2, 0.0 );
        TS_ASSERT_THROWS_NOTHING( sv1.executeChanges() );
    ...
    void test_changes()
    {
        ...
        sv1.commitChange( 0, 2, 2.0 );
        sv1.commitChange( 2, 2, 4.0 );
        sv1.commitChange( 3, 3, 3.0 );
        ...
        sv1.commitChange( 0, 2, 5.0 );
        sv1.commitChange( 2, 0, 1.5 );
    ...
    void test_changes_and_deletions()
    {
        ...
        sv1.commitChange( 1, 0, 5.0 );
        sv1.commitChange( 1, 1, 4.0 );

        sv1.commitChange( 3, 3, 0.0 );
    ...
    void test_transpose()
    {
        ...
        SparseUnsortedVectors sv4;
        sv4.initialize( empty, 2, 3 );

        SparseUnsortedVectors sv5;
