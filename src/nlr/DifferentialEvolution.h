                           ...
                           RandomAccessContainer const &weights )
{
    ...
    for ( size_t i = 0; i < values.size(); ++i )
    {
        if ( weights[i] < 0 || !FloatUtils::isFinite( weights[i] ) )
        {
            std::ostringstream oss;
            oss << __FILE__ << ":" << __LINE__ << ":" << __func__;
            oss << ": All weights must be positive and finite, but got received weight "
                << weights[i] << " at index " << i << " of " << weights.size() << ".";
            throw std::domain_error( oss.str() );
        }
...
// optimization over continuous spaces. Journal of global optimization, 11, 341-359. See:
    ...
    differential_evolution_parameters<ArgumentContainer> const &de_params )
{
    ...
    {
        oss << __FILE__ << ":" << __LINE__ << ":" << __func__;
        oss << ": The population size must be at least 4, but requested population size of "
            << de_params.NP << ".";
        throw std::invalid_argument( oss.str() );
    }

    // From: "Differential Evolution: A Practical Approach to Global Optimization (Natural Computing
    // Series)" > The scale factor, F in (0,1+), is a positive real number that controls the rate at
    // which the population evolves. > While there is no upper limit on F, effective values are
    // seldom greater than 1.0.
    // ...
    // Also see "Limits on F", Section 2.5.1:
    // > This discontinuity at F = 1 reduces the number of mutants by half and can result in erratic
    // convergence...
    ...
    if ( de_params.initial_guess )
    {
        validate_initial_guess(
            *de_params.initial_guess, de_params.lower_bounds, de_params.upper_bounds );
    }
    ...
    std::atomic<std::invoke_result_t<Func, ArgumentContainer>> *current_minimum_cost = nullptr )
{
    ...
    auto population =
        random_initial_population( de_params.lower_bounds, de_params.upper_bounds, NP, gen );
    ...
    for ( size_t generation = 0; generation < de_params.max_generations; ++generation )
    {
        ...
        for ( size_t j = 0; j < threads; ++j )
        {
            thread_pool.emplace_back( [&, j]() {
                ...
                std::uniform_real_distribution<DimensionlessReal> unif01( DimensionlessReal( 0 ),
                                                                          DimensionlessReal( 1 ) );
                ...
                for ( size_t i = j; i < cost.size(); i += threads )
                {
                    ...
                    for ( size_t k = 0; k < dimension; ++k )
                    {
                        ...
                        if ( unif01( tlg ) < de_params.crossover_probability ||
                             k == guaranteed_changed_idx )
                        {
                            auto tmp =
                                population[r1][k] + de_params.mutation_factor *
                                                        ( population[r2][k] - population[r3][k] );
