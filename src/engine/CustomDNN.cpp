...
CustomDNNImpl::CustomDNNImpl(const std::vector<int>& layerSizes, const std::vector<std::string>& activationFunctions, 
        ...
        torch::Tensor weightTensor = torch::tensor(flattened_weights, torch::kFloat).view({layerSizes[i+1], layerSizes[i]});
...
CustomDNNImpl::CustomDNNImpl(NLR::NetworkLevelReasoner& nlr){
    ...
    for (int i = 0; i < numLayers; ++i){
        ...
        if (i < numLayers -1){
            ...
            std::vector<std::vector<float>> layerWeights(nextLayer->getSize(), std::vector<float>(layer->getSize()));
            std::vector<float> layerBiases(layer->getSize());

            for (unsigned j=0; j<nextLayer->getSize(); j++){
                for (unsigned k=0; k< layer->getSize(); ++k){
                    layerWeights[j][k] = static_cast<float>(nlr.getLayer(i)->getWeight(i, k, j));
                }
            }
            for (unsigned j = 0; j < nextLayer->getSize(); ++j) {
     ...
     for (size_t i = 0; i < layerSizes.size() - 1; ++i) {
        linearLayers.push_back(register_module("linear" + std::to_string(i), torch::nn::Linear(layerSizes[i], layerSizes[i+1])));
        std::vector<float> flattenedWeights;
        for (const auto& row : weights[i]) {
            flattenedWeights.insert(flattenedWeights.end(), row.begin(), row.end());
        }

        torch::Tensor weightTensor = torch::tensor(flattenedWeights, torch::kFloat).view({layerSizes[i+1], layerSizes[i]});
        torch::Tensor biasTensor = torch::tensor(biases[i], torch::kFloat);
