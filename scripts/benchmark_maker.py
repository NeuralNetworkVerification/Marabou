...
try:
    ...
    os.mkdir(os.path.join(experiment_path, 'properties/kyle'))
...
for i, arch in enumerate(boeing_architectures):
    ...
    for j, eps_delta in enumerate(boeing_epsilons_deltas[i]):
        ...
        for ind in inds:
            ...
            f.write("{} boeing {} {} {}\n".format(marabou_bin_path, network_name,
                                                  boeing_properties_filename, config_file ) )


###########################  kyle config  ###################################
boeing_networks_dir = cfg.get("kyle", "network_dir")
boeing_architectures = cfg.get("kyle", "architectures").split()
boeing_epsilons_deltas = [[(float(pair.split(',')[0]), float(pair.split(',')[1])) for pair in pairs.split()]
                          for pairs in cfg.get("kyle", "epsilons_deltas").split("/")]
boeing_number_of_benchmarks = [np.fromstring(nums, dtype=int, sep=' ') for nums in cfg.get("kyle", "number_of_benchmarks").split("/")]
boeing_training_data_dir = cfg.get('kyle', 'training_data_dir')

boeing_networks_dir = os.path.abspath(shutil.copytree(boeing_networks_dir, experiment_path + 'networks/kyle/'))
boeing_properties_dir = os.path.abspath(experiment_path + 'properties/kyle/')

print("Kyle's networks dir: {}".format(boeing_networks_dir))
print("Kyle's properties dir: {}".format(boeing_properties_dir))
...
for i, arch in enumerate(boeing_architectures):
    ...
    network_name = os.path.join(boeing_networks_dir, "KJ_{}.pb".format(arch))
    network = Marabou.read_tf(network_name)
